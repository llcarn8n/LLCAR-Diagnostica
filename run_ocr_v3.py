"""Run OCR v3: clean GPU, upload fixed script, run full OCR."""
...
# Upload fixed script
sftp = ssh.open_sftp()
sftp.put('scripts/ocr_parts_tables.py', f'{PROJECT}\\scripts\\ocr_parts_tables.py')
sftp.close()
print('Script uploaded')

# Run full OCR
print('\n=== FULL OCR (351 tables, max_side=1024, cuda:0) ===')
t0 = time.time()
env = 'set PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True &&'
cmd = f'cd /d {PROJECT} && {env} "{PY311}" -u scripts/ocr_parts_tables.py --device cuda:0 --resume 2>&1'
